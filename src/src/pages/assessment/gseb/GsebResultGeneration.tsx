...
import { LayoutGrid, Save, Search, Filter, Users, Download } from 'lucide-react';
...
export function GsebResultGeneration() {
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  ...
                  <th
                    key={sub.id}
                    colSpan={2}
                    className={`px-3 py-3 text-center text-xs font-bold uppercase border-l ${i % 2 === 0 ? 'bg-teal-50 text-teal-700 border-teal-200' : 'bg-green-50 text-green-700 border-green-200'}`}>
                  ...
                  <Fragment key={sub.id}>
                        <th
                      className={`px-2 py-2 text-center text-[10px] font-semibold text-gray-400 uppercase border-l ${i % 2 === 0 ? 'bg-teal-50/50 border-teal-100' : 'bg-green-50/50 border-green-100'}`}>

                          Marks
                        </th>
                        <th
                      className={`px-2 py-2 text-center text-[10px] font-semibold text-gray-400 uppercase ${i % 2 === 0 ? 'bg-teal-50/50' : 'bg-green-50/50'}`}>
                        ...
                        return (
                          <Fragment key={sub.id}>
                              ...
                              className={`px-2 py-3 text-center border-l ${bgCls}`}>
                                ...
                                className={`w-12 py-1.5 border-2 rounded-lg text-center text-xs font-bold outline-none transition-all ${v === '' ? 'border-gray-200 focus:border-teal-400' : valid ? 'border-green-300 bg-green-50 text-green-700' : 'border-red-300 bg-red-50 text-red-700'}`}
                                placeholder="—" />
            ...
            <div className="p-4 border-t bg-gray-50 flex items-center justify-between">
              <div className="flex items-center gap-4 text-sm text-gray-500">
                <span>
                  Avg: <strong className="text-teal-600">{stats.avg}%</strong>
                </span>
                <span>
                  Highest:{' '}
